...
const regions = [
  { name: "HashKey Testnet", nodes: 1, status: "live" },
  { name: "MarketAgent", nodes: 1, status: "online" },
  { name: "AtomicSwap", nodes: 1, status: "active" },
  { name: "Reputation", nodes: 1, status: "tracking" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              <h2 className={`text-6xl md:text-7xl lg:text-[128px] font-display tracking-tight leading-[0.9] transition-all duration-1000 ${
                ...
                Live on
                <br />
                <span className="text-muted-foreground">HashKey.</span>
              </h2>

              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                isVisible ? "opacity-100" : "opacity-0"
              }`}>
                Every request, offer, and settlement is executed on HashKey Chain Testnet
                with fully verifiable on-chain events.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                ...
                <span className="text-2xl text-muted-foreground">services online</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Relayer, pricing agent, and contracts stay synchronized for fast request-to-offer execution.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="block text-sm text-muted-foreground mt-2">Agent availability</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">&lt;15s</span>
              <span className="block text-sm text-muted-foreground mt-2">Target settlement time</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {regions.map((region, index) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">{region.nodes} {region.nodes === 1 ? "service" : "services"}</span>
            </div>
          ))}
        </div>
      </div>
